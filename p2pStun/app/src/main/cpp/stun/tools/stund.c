...
{
  int yes = 1;
  int fd = socket (fam, type, proto);//type:SOCK_DGRAM   proto:IPPROTO_UDP 创建一个UDP socket
  union {
  ...
  {
    case AF_INET:
      addr.in.sin_port = htons (port);//port的默认值是IPPORT_STUN = 3478
      break;
  ...
  if ((type == SOCK_DGRAM) || (type == SOCK_RAW))
  {
    switch (fam)
    {
      case AF_INET:
#ifdef IP_RECVERR
				printf("setsockopt level(选项级别):%d, optname(选项级别):%d", SOL_IP, IP_RECVERR);
...
//  	printf("recvfrom  return len:%lu\n", len);
  ...
  if (validation == STUN_VALIDATION_SUCCESS) {
    agent = newagent;
		printf("validation == STUN_VALIDATION_SUCCESS \n");
  }
  else {
