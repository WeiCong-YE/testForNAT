...
printaddr (const char *str, const struct sockaddr *addr, socklen_t addrlen)
{
  ...
  if (val)
  {
//      sprintf(netMap, "%s:%s",str, gai_strerror (val) );
      LOGE ("getnameinfo failed, ret:%d, %s", val, gai_strerror (val));
  }
  else
  {
      sprintf(g_netMap, "%s:%s", hostbuf, servbuf);
      LOGD ("%s: %s port %s", str, hostbuf, servbuf);
  }
}
...
static int run (int family, const char *hostname, const char *service)
{
  ...
  if (service == NULL)
    service = "3478";
    LOGW("STUN 服务器ip:%s, 端口:%s", hostname, service);
  ret = getaddrinfo (hostname, service, &hints, &res);
  if (ret)
  {
    LOGE("%s (port %s): %s", hostname, service,
             gai_strerror (ret));
    return -1;
  }
